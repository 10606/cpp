...
public:
    ...
    static bool comp_fb(big_int const & a, big_int const & b) 
    {
        ...
        for (size_t i = min_size - 1; i < min_size; --i)
        {
            f_ |= (a.data.big.data[i] > b.data.big.data[i]);
            f &= !(a.data.big.data[i] < b.data.big.data[i]);
        }
        return (fb && (fa || (f && f_)));
    }

    static inline big_int div_bb(big_int const & a, big_int const & b)
    {
        big_int reminder(a);
        big_int res;
        big_int muller;
        ...
        for (size_t i = res.size; i < res.size; --i)
        {
            set_zero(muller);
            uint32_t l = a00_;
            uint32_t r = a11_;
...
big_int sToBI (string s) {
    size_t size = (s.size() + 8) / 9;
}
