...
big_integer & big_integer::operator = (big_integer const & a)
{
    //std::cout << " = " << data.data << " " << a.data.data << std::endl;
...
void big_integer::sub_mul(big_integer & a, const big_integer & b, const uint32_t & c, const size_t & pos)
{
    uint64_t sub = 0;
    size_t i = 0;
    //std::cout << pos << " " << a.size << " " << b.size << std::endl;
    for (; (i != b.size) && (i + pos != a.size); ++i)
    {
        sub += static_cast <uint64_t> (b.data.data[i]) * static_cast <uint64_t> (c);
        //std::cout << sub << "  (" << (sub & big_integer::a11_) << ")  " << a.data.data[i+ pos] << std::endl;
...
bool big_integer::check_abecb(const big_integer & a, const big_integer & b, const uint32_t & c, size_t pos)
{
    ...
    {
        sub += static_cast <uint64_t> (b.data.data[i]) * static_cast <uint64_t> (c);
        //std::cout << "  [" << sub << " (" << (sub & big_integer::a11_) << ") " << a.data.data[i + pos] << "]  ";
    ...
    {
        //std::cout << " is not end of A " << sub << "  " << a.data.data[i + pos] << " " << flag << std::endl;
...
inline big_integer big_integer::div_bb(big_integer & reminder, big_integer const & b, bool type)
{
    ...
    //std::cout << pos << "  " << reminder.size << "  " << b.size << "\n";
...
{
    bool ans = big_integer::comp_fe(a, b) && (a.signed_ == b.signed_ || big_integer::check_zero(a));
    //if (!ans)
...
{
    //std::cout << a << " *= " << b << std::endl;
    //big_integer tmp = big_integer::mul_bb(a, b);
    //a = tmp;
...
{
    //std::cout << a << " /= " << b << std::endl;
    //big_integer tmp = big_integer::div_bb(a, b, 0);
    //a = tmp;
    //std::cout << tmp << std::endl;
    a = a / b;
    return a;
