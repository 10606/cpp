...
big_integer & big_integer::operator = (big_integer const & a)
{
    if (a.data.data == data.data)
        return *this;
    big_integer::set_size(*this, a.size);
    signed_ = a.signed_;
    size = a.size;
...
inline std::pair<uint32_t, uint32_t> const big_integer::mul(uint32_t const & a, uint32_t const & b, uint32_t cf_, uint32_t cf__)   // a * b + cf_
{
    ...
    return std::make_pair((res >> 32), res & big_integer::a11_);
}
...
inline std::string big_integer::bi_to_string(big_integer s)
{
    std::string res = "";
...
inline big_integer big_integer::div_bb(big_integer const & a, big_integer const & b, bool type)
{
    big_integer reminder(a);
    big_integer res;
    big_integer muller;
...
{
    return (a.signed_ == b.signed_ || big_integer::check_zero(a)) &&  big_integer::comp_fe(a, b);
}
bool operator != (big_integer a, big_integer b)
{
...
bool operator > (big_integer a, big_integer b)
{
    return (!a.signed_ || b.signed_) && big_integer::comp_fb(a, b);
}
bool operator <= (big_integer a, big_integer b)
{
    return (a.signed_ || !b.signed_) && !big_integer::comp_fb(a, b);
...
bool operator < (big_integer a, big_integer b)
{
    return (a.signed_ || !b.signed_) && !big_integer::comp_fbe(a, b);
}
bool operator >= (big_integer a, big_integer b)
{
    return (!a.signed_ || b.signed_) && big_integer::comp_fbe(a, b);
...
big_integer operator - (big_integer a, big_integer b)
{
    ...
    else
    {
        ...
        {
            big_integer ans = big_integer::add_bb(b, a, 1);
            ans.signed_ = !ans.signed_;
            return ans;
        }
    }
}
    ...
    data((val.size() + 8) / 9)
{
    ...
    for (; i != val.size(); ++i)
    {
        ...
        big_integer::add_asign(*this, val[i] - '0');
    }
}
